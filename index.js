...
async function startBot() {
  ...
  sock.ev.on("connection.update", async ({ connection, lastDisconnect, qr }) => {
    ...
    if (connection === "open") {
      ...
      // 🚀 Premier démarrage → message spécial + redémarrage auto
      if (!fs.existsSync("./.firstboot")) {
        fs.writeFileSync("./.firstboot", "done");

        const messageTexte = `
🚀 RENEGADES MD V1 TERMINÉ    Bot Disponible ! 🚀
...
By 𝘋𝘌𝘝-𝘙𝘈𝘐𝘡𝘌𝘓
        ...
        console.log(chalk.red("⚠️ Premier démarrage → redémarrage auto dans 5s..."));
  ...
  // === Générer code pairing si pas encore enregistré ===
  setTimeout(async () => {
  ...
  sock.ev.on("messages.upsert", async ({ messages }) => {
    ...
    if (!allowedUsers.includes(senderNum)) {
      return;
    }
    ...
    if (isGroup) {
      ...
      console.log(chalk.yellow(`[PRIVÉ] (${senderNum}) → ${text}`));
    }
